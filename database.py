...
class Database:
    def __init__(self):
        try:
            engine = create_engine('postgres+psycopg2://postgres:1111@192.168.1.234/coursework')
            Session = sessionmaker(bind=engine)
            self.session = Session()
            print("Connected 192.168.1.234")
            self.cursor = None
    ...
    #         self.cursor = self.connection.cursor()
    #
    #         self.engine = create_engine('postgres+psycopg2://postgres:1111@192.168.1.234/coursework')
    ...
    def backup(self):
        ...
        f.write(next_id_str)
        f.close()
        backup_str = f"""C:\\"Program Files"\PostgreSQL\\12\\bin\pg_dump.exe -h 192.168.1.234 -U postgres coursework > %HOMEPATH%\Desktop\DB\coursework\\backups\\{next_id}.dump"""
    ...
    def restore(self):
        try:
            self.__del__()
            print("-"*65)
            os.chdir(r'C:\Users\Nazar\Desktop\DB\coursework\backups')
            fl = glob.glob('*.dump')
            ...
            if backup_id == 0:
                self.__init__()
                return
            os.system(rf'C:\"Program Files"\PostgreSQL\12\bin\dropdb.exe -h 192.168.1.234 -U postgres -e coursework')
            print("Drop\n", "-"*45)
            os.system(f"""C:\\"Program Files"\PostgreSQL\\12\\bin\createdb.exe -h 192.168.1.234 -U postgres -E UTF-8 -e coursework""")
            print("Create\n", "-"*45)
            if not (os.system(f"""C:\\"Program Files"\PostgreSQL\\12\\bin\psql.exe -h 192.168.1.234 -U postgres coursework < %HOMEPATH%\Desktop\DB\coursework\\backups\\{backup_id}.dump""")):
                print("-"*45, "\nSuccessful restore!")
                print("-"*45)
            self.__init__()
            print("-" * 45)
    ...
    def vis(self):
        try:
            f = open("./visualization/next_plt_id.txt", "r")
            next_id = f.read()
            f.close()
            next_id = int(next_id)
            next_id_str = str(next_id + 1)
            f = open("./visualization/next_plt_id.txt", "w")
            f.write(next_id_str)
            f.close()
            print(next_id)
            return next_id
    ...
    def plt_s(self, df):
        id = self.vis()
        plt.savefig(f"./visualization/{id}.png")
        plt.plot(df)
        plt.show()
